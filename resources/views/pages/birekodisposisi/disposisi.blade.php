						...
						<div class="panel-wrapper collapse in">
							<div class="panel-body">
								<div class="row" style="margin-bottom: 10px">
									<form method="GET" action="/{{ env('APP_NAME') }}/disposisi/disposisi">
										<div class=" col-md-2">
													...
													<?php foreach ($dispinboxundangan as $key => $inbox) {
														...
														<tr>
																		...
																		<table class="table table-bordered">
																			<tbody>
																				...
																				<tr>
																					...
																					<td><a href="/{{ env('APP_NAME') }}/disposisi/log?form={{ $thisnoform }}" target="_blank"><button type="submit" class="btn btn-info">Lihat Log</button></a></td>
																				</tr>
																			</tbody>
																		</table> 
																	</span> 
															...
															<td style="vertical-align: middle;">
																...
																<form method="GET" action="/{{ env('APP_NAME') }}/disposisi/lihat disposisi">
											...
											<table id="myTable2" class="table table-hover table-striped" style="z-index: 99999;">
												...
												<tbody>
													<?php foreach ($dispsentundangan as $key => $sent) {
														...
														// if (isset($_SESSION['biroeko_data']['idunit'])) {
														...
														<tr>
															...
															<td>{{ $thisdarisent }}</td>
															<td>{{ $thiskesent }}</td>
															<td class="col-sm-2">{{ $_SESSION['biroeko_data']['idunit'] && strlen($_SESSION['biroeko_data']['idunit']) == 10 ? $thispenanganannow : $thispenanganan }}<br>
																<span class="text-muted">{{ $_SESSION['biroeko_data']['idunit'] && strlen($_SESSION['biroeko_data']['idunit']) == 10 ? $thiscatatannow : $thiscatatan }}</span>
															</td>
															<!-- <td>{{ $_SESSION['biroeko_data']['idunit'] && strlen($_SESSION['biroeko_data']['idunit']) == 10 ? $thiscatatannow : $thiscatatan }}</td> -->
															<td>
																@if($thisrd == 'S')
		...
		$(function () {
			...
			$('.btn-delete-sent').on('click', function () {
				...
				if(confirm("Menghapus disposisi yang sudah berjalan akan menghapus seluruh disposisi dengan nomor form yang sama, lanjutkan?")){
					if (confirm("Apa anda yakin menghapus form dengan nomor "+$el.data('no_form')+" ?")) {
						...
						$.ajax({ 
						type: "GET", 
						url: "/disp-biro-eko/disposisi/form/hapusdisposisi",
			...
			$('.btn-delete').on('click', function () {
				...
				if (confirm("Apa anda yakin menghapus form dengan nomor "+$el.data('no_form')+" ?")) {
					...
					$.ajax({ 
					type: "GET", 
					url: "/disp-biro-eko/disposisi/form/hapusdisposisiemp",
