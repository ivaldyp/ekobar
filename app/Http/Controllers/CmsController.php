...
class CmsController extends Controller
{
	...
	{
        $query = DB::connection('sqlsrv')->table('bpadkobar.dbo.sec_menu')->select();
        if ($parent == 0) {
			$query = $query->where('sao', 0)->orWhereNull('sao');
		} else {
			$query = $query->where('sao', $parent);
		}
        $query->orderBy('urut')->orderBy('ids');
        $query = $query->get();
								...
								'.(($access['zupd'] == 'y' || $access['zdel'] == 'y') ? 
									'<td class="col-md-2">
										'.(($access['zupd'] == 'y') ? 
											...
											<a href="/'.env('APP_NAME').'/cms/menuakses?menu='.$menu['ids'].'&nama='.$menu['desk'].'"><button type="button" class="btn btn-warning"><i class="fa fa-key"></i></button></a>
											'
										: '').'
										'.(($access['zdel'] == 'y') ? 
