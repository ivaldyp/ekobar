...
class DisposisiController extends Controller
{
	...
	public function log(Request $request)
	{
		...
		return view('pages.birekodisposisi.log')
	...
	public function display_disposisi($no_form, $idtop, $level = 0)
	{
		...
		$query = DB::select( DB::raw("SELECT *, biroekodt.dbo.fr_disposisi.tgl as disptgl
					from biroekodt.dbo.fr_disposisi
					left join biroekodt.dbo.emp_data on biroekodt.dbo.emp_data.id_emp = biroekodt.dbo.fr_disposisi.to_pm
					where no_form = '$no_form'
					and idtop = '$idtop'
					and biroekodt.dbo.fr_disposisi.sts = 1
					order by ids
					") );
	...
	public function formdisposisi(Request $request)
	{
		...
		$idgroup = $_SESSION['biroeko_data']['id_emp'];
		if (is_null($idgroup)) {
		...
		return view('pages.birekodisposisi.formdisposisi')
				->with('access', $access)
	...
	public function disposisitambah(Request $request)
	{
		...
		return view('pages.birekodisposisi.disposisitambah')
	...
	public function disposisiubah(Request $request)
	{
		...
		return view('pages.birekodisposisi.disposisiubah')
	...
	public function disposisi(Request $request)
	{
		...
		$dispinboxundangan = DB::select( DB::raw("SELECT TOP (1000) d.[ids]
												  ...
												  ,d.[child]
												  ,m.[catatan_final]
												  FROM [biroekodt].[dbo].[fr_disposisi] d
												  left join biroekodt.dbo.emp_data as emp1 on emp1.id_emp = d.from_pm
												  left join biroekodt.dbo.emp_data as emp2 on emp2.id_emp = d.to_pm
												  left join biroekodt.dbo.fr_disposisi as t on t.ids = d.idtop
												  left join biroekodt.dbo.fr_disposisi as m on m.no_form = d.no_form and m.idtop = 0
		...
		return view('pages.birekodisposisi.disposisi')
				->with('access', $access)
	...
	public function disposisilihat(Request $request)
	{
		$dispmaster = DB::select( DB::raw("SELECT d.[ids]
												  ...
												  left join biroekodt.dbo.emp_data as emp1 on emp1.id_emp = d.from_pm
												  left join biroekodt.dbo.emp_data as emp2 on emp2.id_emp = d.to_pm
												  join biroekodt.dbo.fr_disposisi as m on m.no_form = d.no_form and m.idtop = 0 and m.sts = 1
												  --join biroekodt.dbo.glo_org_unitkerja as unit on unit.kd_unit = d.kd_unit
												  --join biroekodt.dbo.Glo_disposisi_kode as kode on kode.kd_jnssurat = m.kode_disposisi 
		...
		$tujuan = DB::select( DB::raw("SELECT id_emp, tbjab.idunit FROM biroekodt.dbo.emp_data as a
			CROSS APPLY (SELECT TOP 1 tmt_jab,idskpd,idunit,idlok,tmt_sk_jab,no_sk_jab,jns_jab,replace(idjab,'NA::','') as idjab,eselon,gambar FROM  biroekodt.dbo.emp_jab WHERE a.id_emp=emp_jab.noid AND emp_jab.sts='1' ORDER BY tmt_jab DESC) tbjab
			CROSS APPLY (SELECT TOP 1 * FROM biroekodt.dbo.glo_org_unitkerja WHERE glo_org_unitkerja.kd_unit = tbjab.idunit) tbunit
			,biroekodt.dbo.glo_skpd as b,biroekodt.dbo.glo_org_unitkerja as c,biroekodt.dbo.glo_org_lokasi as d WHERE tbjab.idskpd=b.skpd AND tbjab.idskpd+'::'+tbjab.idunit=c.kd_skpd+'::'+c.kd_unit AND tbjab.idskpd+'::'+tbjab.idlok=d.kd_skpd+'::'+d.kd_lok AND a.sts='1' AND b.sts='1' AND c.sts='1' AND d.sts='1' 
		...
		if (strtolower($dispmaster['rd']) == 'n') {
			...
			->update([
				'rd' => 'Y',
				'usr_rd' => ($_SESSION['biroeko_data']['id_emp'] ? $_SESSION['biroeko_data']['id_emp'] : $_SESSION['biroeko_data']['usname']),
				'tgl_rd' => date('Y-m-d'),
			]);
		} else {
			Fr_disposisi::where('ids', $dispmaster['ids'])
		...
		if (isset($_SESSION['biroeko_data']['id_emp'])) {
			$kd_unit = $_SESSION['biroeko_data']['idunit'];
		} else {
			$kd_unit = "01";
		}
		...
		return view('pages.birekodisposisi.disposisilihat')
				->with('signdate', $request->signdate)
				->with('dispmaster', $dispmaster)
				->with('treedisp', $treedisp)
				->with('stafs', $stafs)
